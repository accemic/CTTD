...
-->
# t1 - Initial trace test/demo (example of CTTD tool usage; formerly NexRv)
...
## Results (Nexus Trace compression as displayed by the CTTD tool)
...
* With `-cs 8 -rpt 2` option (With callstack and repeat -best possible compression)
...
## Files
  ...
  CTTD tool related:
    ...
    makelog.txt        - Output of 'make 2>&1 | grep -v -E "^[0-9]+ PC: 0x" >makelog.txt':
                         the decoder prints one line per decoded instruction
                         (5 x 164959 here), which the committed log leaves out
    test.elf           - ELF file (compiled by GNU)
    ./test-OBJD.txt    - Result of objdump -d test.elf >./test-OBJD.txt
    ./test-PCONLY.txt  - Plain PC sequence (from 'main' till call to 'exit(0)')
                         It was created from trace saved by IAR simulator.
			 
  CTTD tool output (all created by running 'make'):
    ...
    ./output/test-PCOUT.txt  - Decoder output (identical as ./test-PCONLY.txt)
